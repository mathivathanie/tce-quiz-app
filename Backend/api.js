...
isActive: {
  ...
  totalTimeSeconds: {
    type: Number,
    default: 3600, // 60 minutes default
    min: 60
  },
  maxTimePerQuestion: {
    type: Number,
    default: 300, // 5 minutes default, only used for hybrid mode
    min: 30
  },
...
app.get('/api/quiz-sessions', async (req, res) => {
  try {
    ...
    const sessions = await QuizSession.find({ createdBy: adminEmail })
      .sort({ createdAt: -1 })
      .select('-__v')
      .lean(); // Use lean for better performance, then add defaults
    
    // Add default values for timer fields if not present
    const sessionsWithDefaults = sessions.map(session => ({
      ...session,
      timerType: session.timerType || 'total',
      totalTimeSeconds: session.totalTimeSeconds || 3600,
      maxTimePerQuestion: session.maxTimePerQuestion || 300
    }));
...
app.post('/api/quiz-sessions', async (req, res) => {
  try {
    const { name, createdBy, adminUserId, timerType = 'total', totalTimeSeconds = 3600, maxTimePerQuestion = 300 } = req.body;
  ...
  try {
    ...
    await violation.save();
       return res.json({
        ...
        originalQuestions: session.questions,
        studentInfo: {
          name: violation.studentName,
          regNo: violation.regNo,
          department: violation.department,
