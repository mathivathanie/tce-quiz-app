...
const Admin = () => {
  ...
  const [totalTimeSeconds, setTotalTimeSeconds] = useState(3600);
  const [maxTimePerQuestion, setMaxTimePerQuestion] = useState(300);
  ...
  const handleOpenSessionModal = () => {
    setSessionName('');
    setTimerType('total');
    setTotalTimeSeconds(3600);
    setMaxTimePerQuestion(300);
...
{showSessionModal && (
  <div style={{
    ...
    <div style={{
      ...
      borderRadius: '24px', padding: '40px', minWidth: '450px', maxWidth: '550px',
      ...
      <p style={{ color: '#9ca3af', textAlign: 'center', marginBottom: '30px' }}>Configure quiz settings</p>
      
      {/* Session Name */}
      <label style={{ color: 'white', fontWeight: '600', display: 'block', marginBottom: '10px' }}>Session Name</label>
      ...
      <label style={{ color: 'white', fontWeight: '600', display: 'block', marginBottom: '12px' }}>Timer Mode</label>
      <div style={{ display: 'flex', gap: '15px', marginBottom: '25px' }}>
        <label style={{ display: 'flex', alignItems: 'center', gap: '8px', color: 'white', cursor: 'pointer' }}>
          <input type="radio" name="timerType" value="total" checked={timerType === 'total'} 
            onChange={(e) => setTimerType(e.target.value)} 
            style={{ cursor: 'pointer' }} />
          <span>Total Time</span>
        </label>
        <label style={{ display: 'flex', alignItems: 'center', gap: '8px', color: 'white', cursor: 'pointer' }}>
          <input type="radio" name="timerType" value="hybrid" checked={timerType === 'hybrid'} 
            onChange={(e) => setTimerType(e.target.value)} 
            style={{ cursor: 'pointer' }} />
      ...
      {/* Total Time */}
      <label style={{ color: 'white', fontWeight: '600', display: 'block', marginBottom: '10px' }}>
        Total Time (minutes) - {timerType === 'hybrid' ? '⚠️ Will be auto-calculated in Hybrid mode' : 'Custom duration'}
      </label>
      <input type="number" min="1" value={Math.round(totalTimeSeconds / 60)} 
        onChange={(e) => setTotalTimeSeconds(parseInt(e.target.value) * 60 || 3600)}
        disabled={timerType === 'hybrid'}
        style={{ width: '100%', padding: '16px 20px', fontSize: '18px', border: '2px solid rgba(187, 134, 252, 0.3)',
          borderRadius: '12px', background: timerType === 'hybrid' ? 'rgba(255, 255, 255, 0.02)' : 'rgba(255, 255, 255, 0.05)', 
          color: 'white', outline: 'none',
          marginBottom: '25px', boxSizing: 'border-box', opacity: timerType === 'hybrid' ? 0.6 : 1 }}
      />

      {/* Max Time Per Question (Only for Hybrid) */}
      {timerType === 'hybrid' && (
        <>
          <label style={{ color: 'white', fontWeight: '600', display: 'block', marginBottom: '10px' }}>
            Max Time Per Question (minutes)
          </label>
          <input type="number" min="1" value={Math.round(maxTimePerQuestion / 60)} 
            onChange={(e) => setMaxTimePerQuestion(parseInt(e.target.value) * 60 || 300)}
            style={{ width: '100%', padding: '16px 20px', fontSize: '18px', border: '2px solid rgba(187, 134, 252, 0.3)',
              borderRadius: '12px', background: 'rgba(255, 255, 255, 0.05)', color: 'white', outline: 'none',
              marginBottom: '12px', boxSizing: 'border-box' }}
          />
          <p style={{ color: '#9ca3af', fontSize: '12px', marginBottom: '25px', fontStyle: 'italic' }}>
            💡 Total time will be auto-calculated as: (number of questions) × (max time per question)
          </p>
        </>
      )}
