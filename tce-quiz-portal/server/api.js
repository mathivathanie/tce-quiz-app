  ...
  async (accessToken, refreshToken, profile, done) => {
    try {
      ...
      user = new User({
        ...
        email: email,
        password: await hash(Math.random().toString(36), 10), // Random password for SSO users
        role: role,
...
app.get('/api/auth/google/callback',
  ...
  (req, res) => {
    ...
    res.redirect(`http://localhost:5173/auth/success?user=${encodeURIComponent(JSON.stringify(userResponse))}`);
  }
);
